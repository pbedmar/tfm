...
\addbibresource{citations/bibliography.bib}
\addbibresource{citations/websites.bib}
...
\setcounter{page}{3}
	...
	The relationships between the factors and electricity prices depends on the data frequency (hourly, daily, monthly) but also on main drivers like: the consumption, the available renewable power (mainly wind and solar), gas and other commodity prices (and also CO2). These models can not only be used to forecast prices, but also to understand how these drivers are influencing them.

	In this Thesis the author has compared the performance of different forecasters, selecting the one who worked better for each data frequency. He has also discovered how different data frequencies lead to different predictor's influences, being those related with gas price and generation the most relevant.
...
\input{chapters/02_planning}
\input{chapters/04_energy_market}
\input{chapters/03_state_of_the_art}
\input{chapters/05_methodology}
\input{chapters/06_analysis}
...
\printbibliography[type=online, title={Other sources}]
